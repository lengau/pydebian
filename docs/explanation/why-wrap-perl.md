...
1. **Behaviour drift** — subtle differences accumulate over time as the Perl
   modules evolve and the Python copy doesn't track changes
2. **Testing burden** — you need to replicate the full test suite of the
   upstream Perl modules
3. **Bug-for-bug compatibility** — tools in the Debian ecosystem expect the
   exact behaviour of the Perl implementations; "almost the same" isn't good
   enough

## The wrapping approach

By embedding Perl and calling the canonical implementations directly:

- **100% behavioural compatibility** — by definition, you get the same answers
  as the Perl tools because you're running the same code
- **Zero maintenance burden** — when `debhelper` or `distro-info-data` gets a
  new release, pydebian automatically picks up the changes (they're loaded from
...
| Advantage | Cost |
|-----------|------|
| Perfect compatibility | Requires Perl + modules installed at runtime |
| No maintenance burden | Depends on perlthon (Rust/PyO3 build dependency) |
| Automatic updates | Cannot work without system packages |
| Battle-tested logic | Slightly higher call overhead than native Python |
...
- You need to run in environments without Perl (e.g. Alpine containers,
  Windows)
